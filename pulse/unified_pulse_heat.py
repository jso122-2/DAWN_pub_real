...
class UnifiedPulseHeat:
    ...
    def __init__(self, decay_rate=0.02, memory_window=100, max_heat=10.0):
        ...
        with self._lock:
            ...
            # Core thermal state
            self.heat = 0.0  # Legacy property name
            self.current_heat = 0.0  # New property name (synced with heat)
            ...
            self.cooling_rate = decay_rate * 2  # Passive cooling
    ...
    def _add_heat_internal(self, source: str, amount: float, context: str = "", legacy_call: bool = False) -> float:
        ...
        with self._lock:
            ...
            old_heat = self.heat
            self.heat = min(self.heat + final_amount, self.heat_capacity)
            self.current_heat = self.heat  # Keep synced
    ...
    def _calculate_stability_index(self) -> float:
        ...
        normalized_fluctuation = fluctuation / max(self.heat_capacity, 1.0)
    ...
    def _check_suppression_triggers(self):
        ...
        if self.heat > self.heat_capacity * 0.95 and not hasattr(self, "last_suppressed"):
            try:
        ...
        if hasattr(self, "last_suppressed") and self.heat < self.heat_capacity * 0.7:
            try:
    ...
    def tick_thermal_update(self) -> Dict[str, float]:
...
def tick_thermal_update() -> Dict[str, float]:
    """Perform per-tick thermal regulation."""
    return pulse.tick_thermal_update()
