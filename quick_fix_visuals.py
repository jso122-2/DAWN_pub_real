...
if not tick_log.exists():
    with open(tick_log, 'w', encoding='utf-8') as f:
        f.write("tick,zone,pulse,entropy,scup,timestamp\n")
        for i in range(1000, 1200):
            zone = ["calm", "active", "surge"][i % 3]  # Remove emojis for compatibility
...
with open(interval_log, 'w', encoding='utf-8') as f:
    for i in range(100):
...
with open(zone_log, 'w', encoding='utf-8') as f:
    for i in range(100):
        tick = 1000 + i
        zone = ["calm", "active", "surge"][i % 3]  # Remove emojis
        heat = 0.3 + (i % 10) * 0.05
