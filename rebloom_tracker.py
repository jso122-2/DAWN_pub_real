...
class RebloomTracker:
    ...
    def create_root_bloom(self, bloom_id: str, initial_entropy: float = 0.0) -> bool:
        """
        Create a root bloom with no parent.
        
        Args:
            bloom_id: ID of the root bloom
            initial_entropy: Initial entropy value
            
        Returns:
            True if successfully created, False if already exists
        """
        if bloom_id in self.nodes:
        ...
        root_node = BloomNode(
            ...
            parent_id=None,
            depth=0,
            total_entropy_drift=initial_entropy
        )
        
        # Update structures
        self.nodes[bloom_id] = root_node
        self.roots.add(bloom_id)
        self.depth_index[0].append(bloom_id)
        self.lineage_graph.add_node(bloom_id, depth=0, entropy_drift=initial_entropy)
        
        print(f"[RebloomTracker] 🌱 Created root bloom: {bloom_id}")
        return True
        
    def log_rebloom(self, child_id: str, parent_id: Optional[str], entropy_diff: float,
        ...
        Args:
            ...
            parent_id: ID of the parent bloom (None for root blooms)
        ...
        """
        # Handle root bloom creation
        if parent_id is None:
            return self.create_root_bloom(child_id, entropy_diff)
