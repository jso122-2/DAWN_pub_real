...
class NeuralSimulator:
    ...
    async def _update_network_activity(self, tick_number: int):
        """Update neural network activity - responds to SCUP, entropy, and mood delta"""
        
        # GET DYNAMIC INPUTS: SCUP, entropy, mood from DAWN's state
        try:
            ...
            current_scup = thermal_profile.get('scup', 0.5)
            
            # Get current entropy 
            current_entropy = thermal_profile.get('entropy', 0.5)
            
            # Get mood delta from thermal state
            mood_indicator = thermal_profile.get('current_zone', 'CALM')
            mood_values = {
                'CALM': 0.2, 'ACTIVE': 0.5, 'INTENSE': 0.8, 
                'SURGE': 0.9, 'CRITICAL': 0.3  # Critical drops due to overwhelm
            }
            current_mood = mood_values.get(mood_indicator, 0.5)
            
        except ImportError:
            # Fallback to tick-based estimation
            current_scup = 0.5 + 0.2 * math.sin(tick_number * 0.05)
            current_entropy = 0.5 + 0.1 * math.cos(tick_number * 0.03)
            current_mood = 0.5 + 0.15 * math.sin(tick_number * 0.07)
        
        # CALCULATE MOOD DELTA: Rate of change in mood state
        mood_delta = 0.0
        if hasattr(self, '_last_mood'):
            mood_delta = current_mood - self._last_mood
        self._last_mood = current_mood
        
        # NEURAL ACTIVITY MODULATION based on SCUP, entropy, mood delta
        # SCUP drives base neural firing rate
        scup_drive = current_scup * 0.8 + 0.1  # Scale to 0.1-0.9
        
        # Entropy adds chaotic fluctuations
        entropy_chaos = current_entropy * 0.3
        
        # Mood delta creates activity spikes
        mood_spike = abs(mood_delta) * 2.0 if abs(mood_delta) > 0.05 else 0.0
        
        # Combined neural modulation
        neural_modulation = scup_drive + entropy_chaos + mood_spike
        
        # Rhythmic patterns modulated by DAWN state
        base_rhythm = 0.1 * math.sin(tick_number * 0.1) * current_scup
        fast_rhythm = 0.05 * math.sin(tick_number * 0.5) * current_entropy
        mood_rhythm = 0.08 * math.sin(tick_number * 0.3) * current_mood
        ...
        for node in self.nodes.values():
            ...
            # Apply SCUP-driven baseline activity
            new_activation += scup_drive * 0.1
            
            # Apply entropy-driven noise
            entropy_noise = np.random.normal(0, current_entropy * 0.05)
            new_activation += entropy_noise
            ...
            # Add modulated rhythmic components
            new_activation += base_rhythm + fast_rhythm + mood_rhythm
            
            # Mood delta spike effect
            if mood_spike > 0:
                spike_probability = mood_spike * 0.2
                if np.random.random() < spike_probability:
                    new_activation += mood_spike * 0.3
            ...
            for connection in self.connections.values():
                if connection.to_node == node.id:
                    ...
                    # Weight connections by SCUP coherence
                    scup_weight = connection.weight * (0.5 + current_scup * 0.5)
                    input_sum += source_node.activation * scup_weight * connection.strength
                    input_count += 1
            ...
            if input_count > 0:
                ...
                new_activation += sigmoid_input * 0.3 * neural_modulation
