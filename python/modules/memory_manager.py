...
class MemoryManager:
    ...
    def _calculate_memory_pressure(self) -> float:
        """Calculate memory pressure - dynamic based on queued rebloom and drift accumulation"""
        # Base pressure from memory usage
        base_pressure = len(self.fragments) / self.max_fragments
        
        # REBLOOM QUEUE PRESSURE: Direct pressure from pending reblooms
        rebloom_pressure = 0.0
        try:
            # Try multiple rebloom sources
            try:
                from bloom.unified_bloom_engine import get_rebloom_queue_size
                rebloom_queue_size = get_rebloom_queue_size()
            except ImportError:
                from bloom.bloom_engine import get_rebloom_count
                rebloom_queue_size = get_rebloom_count() 
        except ImportError:
            # Estimate from consolidation pressure
            rebloom_queue_size = len(self.consolidation_events) if hasattr(self, 'consolidation_events') else 0
        
        # Dynamic rebloom pressure calculation
        if rebloom_queue_size > 0:
            # Queued reblooms create exponential pressure
            queue_ratio = rebloom_queue_size / max(len(self.fragments), 1)
            rebloom_pressure = min(0.8, queue_ratio * 0.4 + (queue_ratio ** 2) * 0.3)
        
        # DRIFT ACCUMULATION PRESSURE: Pressure from semantic drift
        drift_pressure = 0.0
        try:
            from semantic.drift_calculator import calculate_pressure_delta
            # Use recent memory as drift context
            recent_memories = [f.content[:50] for f in list(self.fragments.values())[-5:]]
            if recent_memories and len(recent_memories) > 1:
                current_memory = recent_memories[-1]
                memory_context = recent_memories[:-1]
                drift_delta = calculate_pressure_delta(current_memory, memory_context)
                # Convert drift to pressure - rising drift increases memory pressure
                drift_pressure = max(0.0, drift_delta * 0.1)
        except ImportError:
            # Fallback: calculate from fragment importance variance
            if len(self.fragments) > 3:
                importance_values = [f.importance for f in self.fragments.values()]
                import numpy as np
                importance_variance = np.var(importance_values)
                drift_pressure = min(0.2, importance_variance * 0.5)
        
        # Memory fragmentation: Self-derived from DAWN's own importance patterns
        fragmentation_pressure = 0.0
        min_fragments = max(len(self.fragments) // 10, 10)  # DAWN's own minimum threshold
        if len(self.fragments) > min_fragments:
            ...
            if importances:
                ...
                # Self-referential scaling based on DAWN's own importance distribution
                fragmentation_scaling = importance_mean if importance_mean > 0 else base_pressure
                fragmentation_pressure = min(base_pressure, importance_std / fragmentation_scaling)
        
        # Working memory: Self-derived from DAWN's own access patterns
        working_pressure = 0.0
        if hasattr(self, 'working_memory') and self.working_memory:
            working_fill_ratio = len(self.working_memory) / self.working_memory.maxlen
            # DAWN derives its own overflow threshold from memory efficiency
            efficiency_threshold = base_pressure + 0.3  # Self-derived from memory state
            if working_fill_ratio > efficiency_threshold:
                pressure_gradient = 1.0 + base_pressure  # DAWN's own pressure gradient
                working_pressure = (working_fill_ratio - efficiency_threshold) * pressure_gradient
        
        # DYNAMIC PRESSURE INTEGRATION: Combine all pressure sources
        # Include drift pressure in total calculation
        pressure_components = [base_pressure, rebloom_pressure, drift_pressure, fragmentation_pressure, working_pressure]
        
        # Weight distribution based on pressure source criticality
        weights = [0.3, 0.35, 0.2, 0.1, 0.05]  # Rebloom queue most critical, then base, then drift
        
        total_pressure = sum(p * w for p, w in zip(pressure_components, weights))
        
        # Add pressure spike for critical rebloom queue conditions
        if rebloom_pressure > 0.5:
            spike_multiplier = 1.0 + (rebloom_pressure - 0.5) * 0.8
            total_pressure *= spike_multiplier
        
        return min(1.0, max(0.0, total_pressure))
