...
class DAWNImportFixer:
    ...
    def fix_all_imports(self):
        ...
        print("🔧 DAWN Import Dependency Fixer")
        print("=" * 40)
        ...
        print(f"\n📁 Found {len(problematic_files)} files with import issues")
        
        for file_path in problematic_files:
            self.fix_file_imports(file_path)
        
        print(f"\n✅ Applied {len(self.fixes_applied)} import fixes")
...
try:
    ...
    print(f"⚠️ Could not import {primary_import} from {primary_module}: {{e}}")
    ...
    def fix_mock_imports(self, content):
        ...
        mock_patterns = [
            (r"# Mock import removed - using direct import
from (.+) import (.+)", r"# Mock import removed - using direct import\nfrom \1 import \2"),
            (r"# Mock import removed - using direct import
import (.+) as (.+)", r"# Mock import removed - using direct import\nimport \1 as \2")
        ]
    ...
    def fix_helix_imports(self, content):
        ...
        helix_patterns = [
            r"# Helix import removed to fix circular dependency",
            r"helix_import\([^)]+\)",
            r"# Helix import removed to fix circular dependency"
        ]
...
def fix_web_server_imports():
    ...
    print("\n🌐 Fixing web_server.py specific import issues...")
    
    web_server_path = Path("web_server.py")
    if not web_server_path.exists():
        print("  ⚠️ web_server.py not found in current directory")
        return
    ...
    self.fractal_integration = None"""
        ...
        if content != original_content:
            ...
            print("  ✅ Fixed web_server.py import issues")
        else:
            print("  ℹ️ web_server.py already has clean imports")
            
    except Exception as e:
        print(f"  ❌ Error fixing web_server.py: {e}")

def main():
    """Main fix function"""
    print("🚀 DAWN GUI Import Dependency Fixer")
    ...
    print()
    print("✅ Import fixes complete! GUI should now initialize properly.")
