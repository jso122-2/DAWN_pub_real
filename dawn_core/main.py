...
try:
    from cognitive.forecasting_engine import compute_forecast
    from cognitive.forecasting_models import Passion, Acquaintance
    from cognitive.symbolic_anatomy import generate_commentary
    from core.memory.memory_routing_system import MemoryRouter
    from core.memory.cognitive_router import SymbolicRouter
    ...
    def compute_forecast(passion, acquaintance, current_entropy, context=""):
...
class DAWNCognitiveEngine:
    ...
    def _initialize_subsystems(self):
        ...
        try:
            if COGNITIVE_AVAILABLE:
                ...
                self._symbolic_router = SymbolicRouter()
            else:
    ...
    def extract_passion_acquaintance(self, memory_chunk: Dict[str, Any]) -> tuple:
        ...
        try:
            ...
            passion = Passion(
                ...
                topic=topic,
                emotional_valence=0.5 + random.random() * 0.4,  # Slight positive bias
                time_decay=0.1
            )
            
            # Create Acquaintance based on speaker
            speaker = memory_chunk.get("speaker", "unknown")
            familiarity = 0.8 if speaker == "system" else 0.6
            
            acquaintance = Acquaintance(
                familiarity=familiarity,
                emotional_charge=passion.emotional_valence * 0.8,
                trust_level=0.7,
                interaction_history=[topic]
            )
    ...
    def tick(self):
        ...
        try:
            ...
            commentary = generate_commentary(state)
            ...
            limit_horizon = forecast.get('limit_horizon', 0.5)
            if limit_horizon < 0.3 and self.symbolic_router:
    ...
    def _trigger_rebloom(self, forecast: Dict[str, Any], state: Dict[str, Any]):
        ...
        try:
            if self.symbolic_router:
                rebloom_result = self.symbolic_router.rebloom_trigger(
