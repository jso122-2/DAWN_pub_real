...
export function NeuralRadarSystem() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationRef = useRef<number>();
  
  // Use real DAWN consciousness data
  const { data: consciousnessData } = useConsciousness();
  const { 
    tickData, 
    getRecentHistory, 
  ...
  // Calculate derived metrics from DAWN data
  const calculateCoherence = useMemo(() => {
    if (!tickData) return 0.8;
    
    // Coherence = inverse of entropy variance + scup stability
    const recentHistory = getRecentHistory(20);
    if (recentHistory.length < 5) return 0.8;
    
    const entropyValues = recentHistory.map(d => d.entropy);
    const scupValues = recentHistory.map(d => d.scup);
    
    const entropyVariance = entropyValues.reduce((sum, val, idx) => {
      const mean = entropyValues.reduce((s, v) => s + v, 0) / entropyValues.length;
      return sum + Math.pow(val - mean, 2);
    }, 0) / entropyValues.length;
    
    const scupStability = 1 - Math.abs(scupValues[scupValues.length - 1] - scupValues[0]);
    
    return Math.max(0, Math.min(1, 0.5 + scupStability * 0.3 - entropyVariance * 0.5));
  }, [tickData, getRecentHistory]);
  
  const calculateDrift = useMemo(() => {
    if (!tickData) return 0.3;
    
    // Drift = rate of change across all metrics
    const recentHistory = getRecentHistory(10);
    if (recentHistory.length < 3) return 0.3;
    
    const scupDrift = Math.abs(recentHistory[recentHistory.length - 1].scup - recentHistory[0].scup);
    const entropyDrift = Math.abs(recentHistory[recentHistory.length - 1].entropy - recentHistory[0].entropy);
    const heatDrift = Math.abs(recentHistory[recentHistory.length - 1].heat - recentHistory[0].heat);
    
    const totalDrift = (scupDrift + entropyDrift + heatDrift) / 3;
    return Math.max(0, Math.min(1, totalDrift * 2)); // Scale to 0-1
  }, [tickData, getRecentHistory]);
  
  const calculateResonance = useMemo(() => {
    if (!tickData) return 0.65;
    
    // Resonance = correlation between scup and heat + mood stability
    const scupHeatCorrelation = Math.abs(calculateCorrelation('scup', 'heat'));
    const moodStabilityFactor = currentTrend === 'stable' ? 0.8 : 
                                currentTrend === 'rising' ? 0.6 : 0.4;
    
    return Math.max(0, Math.min(1, scupHeatCorrelation * 0.6 + moodStabilityFactor * 0.4));
  }, [tickData, calculateCorrelation, currentTrend]);
  
  // Update metrics with real DAWN data
  useEffect(() => {
    if (!tickData) return;
    
    const newMetrics: ProcessMetrics = {
      scup: tickData.scup, // Already 0-1 from WebSocketService
  ...
  }, [tickData, calculateCoherence, calculateDrift, calculateResonance]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Draw center glow with real tick data
      const glowSize = 20 + Math.sin(time * 0.02) * 10;
      const glowIntensity = tickData ? (tickData.scup * 0.5 + 0.3) : 0.3;
  ...
  }, [radarPoints, selectedProcess, showSpiral, showOptimalZones, historical, tickData]);
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <p style={styles.subtitle}>
          Connected to DAWN Tick Engine • {tickData ? `Tick #${tickData.tick_count}` : 'Connecting...'}
        </p>
