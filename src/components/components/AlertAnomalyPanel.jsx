...
const AlertAnomalyPanel = () => {
  ...
  useEffect(() => {
    ...
    // Event bus listeners
    const handleDisturbance = (e) => {
      const disturbance = e.detail;
      setAlerts(prev => [
        {
          id: disturbance.id,
          type: 'Cognitive Disturbance',
      ...
      setPatterns(prev => [
        {
          ...
          message: `${pattern.type} detected (confidence: ${pattern.confidence}%)`,
      ...
      setAnomalies(prev => [
        {
          ...
          confidence: 80,
          location: anomaly.severity || 'Unknown',
          timestamp: new Date()
        },
        ...prev.slice(0, 29)
      ]);
    ...
    const handleThreshold = (e) => {
      ...
      setAlerts(prev => [
        {
          ...
          category: 'threshold',
          message: `Cognitive load ${alert.status} (${(alert.average * 100).toFixed(1)}%)`,
      ...
      createRipple(alert.status === 'Critical' ? 'critical' : 'high');
    };
    eventBus.addEventListener('cognitive-disturbance', handleDisturbance);
    eventBus.addEventListener('pattern-detected', handlePattern);
    eventBus.addEventListener('anomaly-detected', handleAnomaly);
    eventBus.addEventListener('threshold-alert', handleThreshold);
    return () => {
      clearInterval(alertInterval);
      clearInterval(anomalyInterval);
      clearInterval(predictiveInterval);
      eventBus.removeEventListener('cognitive-disturbance', handleDisturbance);
      eventBus.removeEventListener('pattern-detected', handlePattern);
      eventBus.removeEventListener('anomaly-detected', handleAnomaly);
      eventBus.removeEventListener('threshold-alert', handleThreshold);
    };
  }, [predictiveMode]);
