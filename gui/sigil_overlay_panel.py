...
class SigilOverlayPanel(ttk.Frame):
    ...
    def generate_sigils_from_dawn_state(self, dawn_data):
        """
        Generate sigils based on DAWN system state
        
        Args:
            dawn_data: Dictionary containing DAWN cognitive state
        """
        sigils = []
        
        # Extract key metrics
        entropy = dawn_data.get('entropy', 0)
        heat = dawn_data.get('heat', 0)
        scup = dawn_data.get('scup', 0)
        mood = dawn_data.get('mood', 'CONTEMPLATIVE')
        
        # Generate entropy-based sigils
        if entropy > 400000:  # High entropy
            sigils.append({
                "symbol": "🌀",
                "name": "EntropyVortex",
                "house": "🐋",
                "heat": min(100, entropy // 5000),
                "decay": 0.8,
                "source": f"Entropy #{entropy}"
            })
        
        # Generate heat-based sigils
        if heat > 50000:  # High heat
            sigils.append({
                "symbol": "🔥",
                "name": "ThermalSpike",
                "house": "🐝",
                "heat": min(100, heat // 1000),
                "decay": 0.7,
                "source": f"Heat #{heat}"
            })
        
        # Generate SCUP-based sigils
        if scup > 40:  # High coherence
            sigils.append({
                "symbol": "💫",
                "name": "CoherenceBeam",
                "house": "🦉",
        ...
        # Mood-based sigils
        mood_sigils = {
            'CREATIVE': {"symbol": "✨", "name": "CreativeFlash", "house": "🐝"},
            'CONTEMPLATIVE': {"symbol": "🔮", "name": "DeepThought", "house": "🦉"},
            'FOCUSED': {"symbol": "🎯", "name": "LaserFocus", "house": "🐜"},
            'ONLINE': {"symbol": "⚡", "name": "SystemActive", "house": "🐋"}
        }
        
        if mood in mood_sigils:
            mood_sigil = mood_sigils[mood].copy()
            mood_sigil.update({
                "heat": random.randint(30, 80),
                "decay": random.uniform(0.5, 0.9),
                "source": f"Mood: {mood}"
            })
            sigils.append(mood_sigil)
        
        # Add some baseline activity sigils
        for i in range(2, 6):  # 2-5 additional sigils
            sigils.append({
                "symbol": random.choice(["🔁", "🔄", "💭", "🧠", "🌐"]),
                "name": random.choice(["MemoryPulse", "ThoughtLoop", "CognitiveNet", "SynapseFlow"]),
                "house": random.choice(list(self.houses.keys())),
                "heat": random.randint(20, 60),
                "decay": random.uniform(0.3, 0.8),
                "source": f"Background #{i}"
            })
        
        self.update_sigils(sigils)
