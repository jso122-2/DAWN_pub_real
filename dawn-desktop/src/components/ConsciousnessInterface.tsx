...
export function ConsciousnessInterface() {
  const { systemUnity, updateConsciousnessCoherence } = useCosmicStore()
  ...
  return (
    ...
    >
      <div className="glass-heavy rounded-3xl p-8 glow">
        <h2 className="text-3xl font-bold mb-8 text-center text-cyan-300">Consciousness State Monitor</h2>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="glass rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-cyan-300">{(systemUnity * 100).toFixed(1)}%</div>
          </div>
          <div className="glass rounded-xl p-4">
            <div className="text-gray-400 text-sm">Correlation</div>
            <div className="text-2xl font-bold text-blue-300">{(systemUnity * 87).toFixed(1)}%</div>
          </div>
        </div>

        {/* Control */}
        <div>
          <label className="text-gray-400 text-sm">Consciousness Coherence</label>
          <input
            type="range"
            min="0"
            max="1"
            step="0.01"
            value={systemUnity}
            onChange={(e) => updateConsciousnessCoherence(parseFloat(e.target.value))}
