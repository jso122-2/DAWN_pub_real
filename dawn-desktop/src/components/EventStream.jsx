...
import { 
  FaFilter, FaSearch, FaDownload, FaTimes, FaPlay, FaPause,
  FaExclamationTriangle, FaBrain, FaSparkles, FaSyncAlt,
  FaChartLine, FaLink, FaClock, FaChevronRight
} from 'react-icons/fa';
...
const EventStream = ({ events = [], onEventSelect }) => {
  const [filteredEvents, setFilteredEvents] = useState([]);
  const [selectedTypes, setSelectedTypes] = useState(new Set(Object.keys(EVENT_TYPES)));
  const [searchTerm, setSearchTerm] = useState('');
  const [timeRange, setTimeRange] = useState({ start: null, end: null });
  const [isPaused, setIsPaused] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState(null);
  const [showFilters, setShowFilters] = useState(false);
  const [causalLinks, setCausalLinks] = useState(new Map());
  ...
  }, [events, selectedTypes, searchTerm, timeRange]);
      ...
      >
        ...
        {hasChildren && (
          <div className="event-has-effects">
            <FaChevronRight className="effects-icon" />
