...
interface TimelineEvent {
  id: string;
  type: keyof typeof EventTypes | string;
  timestamp: number;
  data: any;
  config: {
    color: string;
...
const NeuralTimeline = ({ maxEvents = 50, timeWindow = 30000 }: { maxEvents?: number; timeWindow?: number }) => {
  ...
  const addEvent = (type: string, data: any) => {
    ...
    setEvents((prevEvents: TimelineEvent[]) => {
  ...
  useEffect(() => {
    ...
    Object.values(EventTypes).forEach((eventType) => {
      eventBus.addEventListener(eventType, handleEvent as EventListener);
    });
    // Add initial event
    addEvent(EventTypes.STATE_CHANGE, { state: 'Neural Timeline Initialized' });
    return () => {
      Object.values(EventTypes).forEach((eventType) => {
        eventBus.removeEventListener(eventType, handleEvent as EventListener);
      });
    };
  }, []);
