...
const EntropyGauge = ({ value, history = [], thresholds = [0.3, 0.7] }) => {
  ...
  const SparkLine = ({ data, width = 100, height = 20 }) => {
...
const TickRateGauge = ({ value, history = [] }) => {
  const [displayValue, setDisplayValue] = useState(0);
...
const CompositeMetrics = ({ scup, entropy, heat, tickRate }) => {
...
const HistoricalCharts = ({ metrics }) => {
  const MiniChart = ({ title, data, color = '#3b82f6', correlations = [] }) => {
