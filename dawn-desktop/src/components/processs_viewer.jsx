...
const ProcessesViewerTab = ({ onToggleComponent }) => {
  ...
  const togglePythonProcess = async (processId) => {
    ...
    try {
      if (process.enabled) {
        // Stop the process
        await invoke('stop_python_process', { script: process.script });
      } else {
        // Start the process with parameters
        await invoke('start_python_process', { 
  ...
  const toggleReactComponent = (componentId) => {
  ...
  const togglePipelineModule = (moduleIndex) => {
    setPythonProcesses(prev => prev.map(proc => {
  ...
  // Calculate totals
  const allProcesses = [...pythonProcesses, ...reactComponents];
          ...
          <button 
            onClick={() => {
              allProcesses.forEach(proc => {
                if (!proc.enabled) {
                  if (proc.script) togglePythonProcess(proc.id);
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-3 flex-1">
                <button
                  onClick={() => process.script ? togglePythonProcess(process.id) : toggleReactComponent(process.id)}
