...
# REMOVED: Format string monkey patch that was causing immutable type error
# str.format = safe_str_format  # <-- This line removed per emergency fix

# Core consciousness systems with fallback handling
try:
    print("✅ Pulse heat system loaded")
except ImportError as e:
    print(f"⚠️ Pulse heat import error: {e}")
    # Fallback minimal pulse system
    class FallbackPulse:
        def __init__(self):
            self.heat = 0.0
        ...
        def add_heat(self, amount, source, reason=""): 
            self.heat = max(0, min(10, self.heat + amount))
            print(f"[Pulse] +{amount} heat added: {reason}")
    
    pulse = FallbackPulse()
    def tick_thermal_update(): 
        return pulse.get_thermal_profile()
