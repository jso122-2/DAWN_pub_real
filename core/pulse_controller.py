...
class PulseController:
    ...
    def update_heat(self, new_heat_value: float) -> Dict[str, Any]:
        ...
        try:
            ...
            # Sync with entropy analyzer if available
            self._sync_entropy_data(previous_heat, self.current_heat, zone_changed)
    ...
    def set_entropy_analyzer(self, entropy_analyzer) -> None:
        """
        Set the entropy analyzer for thermal-entropy correlation
        
        Args:
            entropy_analyzer: EntropyAnalyzer instance
        """
        self.entropy_analyzer = entropy_analyzer
        print(f"🧬 Entropy analyzer connected to pulse controller")
    
    def _sync_entropy_data(self, previous_heat: float, current_heat: float, zone_changed: bool) -> None:
        """
        Sync thermal data with entropy analyzer
        
        Args:
            previous_heat: Previous heat value
            current_heat: Current heat value  
            zone_changed: Whether zone transition occurred
        """
        try:
            ...
            if current_time - self.last_entropy_sync < 5.0:
                return
            
            # Convert thermal data to entropy metrics
            thermal_data = {
                'heat': current_heat,
            ...
            # Inject thermal awareness into entropy analyzer
            self.entropy_analyzer.inject_thermal_awareness(thermal_data)
            
            # Get entropy recommendations
            if hasattr(self.entropy_analyzer, 'get_chaos_alerts'):
                alerts = self.entropy_analyzer.get_chaos_alerts('high')
                if alerts:
                    print(f"🚨 Chaos alerts detected: {len(alerts)} high-risk blooms")
                    
                    # Apply thermal regulation if critical alerts
                    critical_alerts = [a for a in alerts if a.risk_level == 'critical']
                    if critical_alerts and current_heat > 60.0:
                        print(f"🔥 Auto-regulating heat due to critical entropy alerts")
                        self.regulate_heat(target_heat=45.0, regulation_speed=0.3)
            
            self.last_entropy_sync = current_time
            
        except Exception as e:
            print(f"❌ Error syncing entropy data: {e}")
    
    def get_entropy_correlated_heat(self) -> Dict[str, Any]:
        """
        Get heat metrics correlated with entropy analysis
        
        Returns:
            Dictionary with thermal-entropy correlation data
        """
        result = {
            'current_heat': self.current_heat,
            'current_zone': self.current_zone,
            'entropy_analyzer_connected': self.entropy_analyzer is not None,
            'entropy_feedback_enabled': self.entropy_feedback_enabled
        }
        
        if self.entropy_analyzer:
            try:
                # Get thermal-influenced blooms
                hot_blooms = self.entropy_analyzer.get_hot_blooms(threshold=0.6)
                at_risk_blooms = self.entropy_analyzer.recommend_stabilization()
                
                result.update({
                    'hot_blooms_count': len(hot_blooms),
                    'at_risk_blooms_count': len(at_risk_blooms),
                    'thermal_entropy_correlation': True
                })
                
                # Get alerts
                if hasattr(self.entropy_analyzer, 'get_chaos_alerts'):
                    alerts = self.entropy_analyzer.get_chaos_alerts()
                    result['chaos_alerts'] = len(alerts)
                    result['critical_alerts'] = len([a for a in alerts if a.risk_level == 'critical'])
