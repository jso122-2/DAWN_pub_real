...
def compute_pressure(state: Dict[str, Any]) -> float:
    """
    Compute normalized pressure value (0-100+ range)
    
    Args:
        state: Dictionary containing bloom_mass, sigil_velocity, etc.
        
    Returns:
        Pressure value (0.0 to 100+)
    """
    engine = get_dawn_formula_engine()
    reading = engine.calculate_pressure(state)
    return reading.pressure_value

def compute_shi(state: Dict[str, Any]) -> float:
    """
    Compute System Health Index (0.0-1.0 range)
    
    Args:
        state: Dictionary containing system health indicators
        
    Returns:
        SHI value (0.0 to 1.0, where 1.0 is optimal health)
    """
    # Extract health indicators
    entropy = state.get('entropy', 0.5)
    pressure = state.get('pressure', 50.0)
    memory_health = state.get('memory_health', 0.7)
    sigil_stability = state.get('sigil_stability', 0.8)
    
    # Calculate SHI based on multiple factors
    # Lower entropy = better health
    entropy_factor = 1.0 - entropy
    
    # Lower pressure = better health (normalized to 0-1)
    pressure_factor = max(0.0, 1.0 - (pressure / 100.0))
    
    # Memory health (0-1)
    memory_factor = memory_health
    
    # Sigil stability (0-1)
    sigil_factor = sigil_stability
    
    # Weighted average
    shi = (entropy_factor * 0.3 + 
           pressure_factor * 0.3 + 
           memory_factor * 0.2 + 
           sigil_factor * 0.2)
