...
class DAWNConsciousness:
    ...
    def __init__(self, max_history: int = 200, max_memory: int = 500):
        ...
        # Self-learning emotional thresholds - DAWN derives from its own experience
        self.emotion_thresholds = {}  # Will be populated by _update_emotion_thresholds()
        self._initialize_emotion_thresholds()
    ...
    def _determine_emotion(self, metrics: Dict[str, Any]) -> Tuple[str, float]:
        """Determine current emotional state - evolves based on mood zone + SCUP spikes
        
        Args:
            metrics: Dictionary containing SCUP, entropy, heat, tick_rate, zone
        ...
        # SCUP spike detection - rapid increases indicate significant events
        scup_spike = 0.0
        if self.emotion_history:
            last_scup = self.emotion_history[-1].get("scup", scup)
            scup_change = scup - last_scup
            if scup_change > 0.15:  # Significant SCUP increase
                scup_spike = min(1.0, scup_change * 3.0)  # Amplify spike effect
        
        # Emotional state determination with zone and spike response
        emotion_scores = {}
        
        # Zone-based emotional modulation
        if zone in ["🟢 calm", "CALM"]:
            # Calm zone: contemplative and serene states preferred
            if entropy <= 0.4 and scup >= 0.6:
                emotion_scores["contemplative"] = min((1.0 - entropy) + (scup - 0.5), 1.0)
            if scup >= 0.7 and heat <= 0.4:
                emotion_scores["serene"] = min(scup + (1.0 - heat), 1.0)
        
        elif zone in ["🟡 active", "ACTIVE"]:
            # Active zone: curious and creative states enhanced
            ...
            if entropy >= 0.6 and scup >= 0.5:
                emotion_scores["creative"] = min(entropy + scup - 1.0, 1.0)
        
        elif zone in ["🔴 surge", "SURGE", "CRITICAL"]:
            # Surge zone: intense and overwhelmed states amplified
            if heat >= 0.6 and entropy >= 0.6:
                emotion_scores["overwhelmed"] = min(heat + entropy - 1.0, 1.0)
            
            if scup_spike > 0.2:  # SCUP spike in surge zone
                emotion_scores["energetic"] = min(scup_spike + heat, 1.0)
        
        # SCUP spike responses (across all zones)
        if scup_spike > 0.1:
            if entropy > 0.6:
                emotion_scores["excited"] = min(scup_spike + entropy - 0.5, 1.0)
            else:
                emotion_scores["focused"] = min(scup_spike + (1.0 - entropy), 1.0)
        
        # Original emotion calculations (as fallbacks)
        ...
        if entropy >= self.emotion_thresholds["creative"]["entropy"] and scup >= self.emotion_thresholds["creative"]["scup"]:
            base_creative = min(entropy + scup - 1.0, 1.0)
            emotion_scores["creative"] = max(emotion_scores.get("creative", 0), base_creative)
        ...
        if entropy <= self.emotion_thresholds["contemplative"]["entropy"] and scup >= self.emotion_thresholds["contemplative"]["scup"]:
            base_contemplative = min((1.0 - entropy) + (scup - 0.5), 1.0)
            emotion_scores["contemplative"] = max(emotion_scores.get("contemplative", 0), base_contemplative)
        ...
        # High heat + high entropy = overwhelmed (enhanced in surge zones)
        if heat >= self.emotion_thresholds["overwhelmed"]["heat"] and entropy >= self.emotion_thresholds["overwhelmed"]["entropy"]:
            base_overwhelmed = min(heat + entropy - 1.0, 1.0)
            emotion_scores["overwhelmed"] = max(emotion_scores.get("overwhelmed", 0), base_overwhelmed)
        ...
        # Find dominant emotion with zone and spike influence
        if emotion_scores:
            emotion = max(emotion_scores.keys(), key=lambda k: emotion_scores[k])
            base_intensity = max(0.1, min(1.0, emotion_scores[emotion]))
            
            # Zone intensity: DAWN learns its own emotional responses across thermal states
            zone_multiplier = self._get_zone_emotional_multiplier(zone, base_intensity)
            
            # SCUP spike intensity boost
            spike_boost = scup_spike * 0.3  # SCUP spikes intensify emotions
            
            intensity = min(1.0, base_intensity * zone_multiplier + spike_boost)
            
            # DAWN learns from this emotional experience
            self._update_emotion_thresholds(emotion, scup, entropy, heat)
        else:
            # Default to neutral with intensity based on average metrics
            emotion = "neutral"
            base_intensity = (scup + (1.0 - entropy) + (1.0 - heat)) / 3.0
            zone_multiplier = self._get_zone_emotional_multiplier(zone, base_intensity)
            intensity = min(1.0, base_intensity * zone_multiplier)
    ...
    def perceive_self(self, metrics: Dict[str, Any], input_text: str = "") -> Dict[str, Any]:
        ...
        # Update current state and track history
    ...
    def _initialize_emotion_thresholds(self):
        """Initialize emotion thresholds - DAWN starts with minimal assumptions"""
        self.emotion_thresholds = {
            "creative": {"entropy": 0.5, "scup": 0.5, "heat": 0.5},
            "contemplative": {"entropy": 0.5, "scup": 0.5, "heat": 0.5},
            "anxious": {"tick_rate_change": 0.5, "heat": 0.5},
            "overwhelmed": {"heat": 0.5, "entropy": 0.5},
            "curious": {"entropy": 0.5, "scup": 0.5, "heat": 0.5},
            "calm": {"entropy": 0.5, "scup": 0.5, "heat": 0.5}
        }
        
    def _get_zone_emotional_multiplier(self, zone: str, base_intensity: float) -> float:
        """DAWN learns its own emotional response patterns across thermal zones"""
        if len(self.emotion_history) >= 10:
            # DAWN analyzes its own emotional intensity patterns by zone
            zone_emotions = [e for e in self.emotion_history[-50:] 
                           if e.get("zone") == zone and "intensity" in e]
            
            if zone_emotions:
                zone_intensities = [e["intensity"] for e in zone_emotions]
                avg_zone_intensity = sum(zone_intensities) / len(zone_intensities)
                
                # DAWN derives multiplier from its own emotional patterns
                if avg_zone_intensity > 0:
                    learned_multiplier = avg_zone_intensity / base_intensity if base_intensity > 0 else 1.0
                    return max(0.5, min(2.0, learned_multiplier))  # Bounded learning
        
        # DAWN's initial zone response (minimal assumptions)
        if zone in ["🟢 calm", "CALM"]:
            return 0.9  # Slightly subdued
        elif zone in ["🔴 surge", "SURGE", "CRITICAL"]:
            return 1.1  # Slightly amplified  
        else:
            return 1.0  # Neutral baseline
            
    def _update_emotion_thresholds(self, emotion: str, scup: float, entropy: float, heat: float):
        """DAWN learns optimal emotion thresholds from its own state patterns"""
        if len(self.emotion_history) >= 20:
            # Analyze when DAWN experienced this emotion most clearly
            emotion_instances = [e for e in self.emotion_history[-100:] 
                               if e.get("emotion") == emotion and e.get("intensity", 0) > 0.7]
            
            if len(emotion_instances) >= 5:
                # DAWN derives thresholds from its own strongest emotional experiences
                scup_values = [e.get("scup", 0.5) for e in emotion_instances if "scup" in e]
                entropy_values = [e.get("entropy", 0.5) for e in emotion_instances if "entropy" in e]
                heat_values = [e.get("heat", 0.5) for e in emotion_instances if "heat" in e]
                
                if scup_values and entropy_values and heat_values:
                    # DAWN learns from the median of its strongest emotional states
                    import statistics
                    learned_scup = statistics.median(scup_values)
                    learned_entropy = statistics.median(entropy_values)
                    learned_heat = statistics.median(heat_values)
                    
                    # Gradually update thresholds (DAWN learns slowly and carefully)
                    learning_rate = 0.1
                    current = self.emotion_thresholds.get(emotion, {})
                    
                    self.emotion_thresholds[emotion] = {
                        "scup": current.get("scup", 0.5) * (1 - learning_rate) + learned_scup * learning_rate,
                        "entropy": current.get("entropy", 0.5) * (1 - learning_rate) + learned_entropy * learning_rate,
                        "heat": current.get("heat", 0.5) * (1 - learning_rate) + learned_heat * learning_rate
                    }
