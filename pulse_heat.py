...
class UnifiedPulseHeat:
    ...
    def __init__(self, decay_rate=0.02, memory_window=100, max_heat=10.0):
        ...
        with self._lock:
            ...
            print(f"[PulseHeat] 🔥 Enhanced thermal system initialized | ID: {id(self)}")
    ...
    def add_heat(self, amount: float, source: str, reason: str = ""):
        """Enhanced add_heat with expression affinity tracking"""
        ...
        if source not in self.heat_sources:
            ...
            if source_type in self.source_expression_affinity:
        ...
        print(f"[PulseHeat] +{amount:.2f} from {source}{reason_str} | Heat: {self.heat:.2f}")
    ...
    def tick_update(self) -> Dict[str, float]:
        """Enhanced tick update with expression dynamics"""
        with self._lock:
            self.tick_count += 1
