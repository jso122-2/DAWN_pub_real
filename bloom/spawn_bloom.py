...
def safe_import_fractal():
    try:
        from enhanced_fractal_system import EnhancedJuliaFractal
        fractal_gen = EnhancedJuliaFractal()
        generate_julia_image = fractal_gen.generate_enhanced_julia
...
# Add these functions to your existing spawn_bloom.py file
# Place them after the existing functions but before the if __name__ == "__main__": section

def spawn_bloom(bloom_data: Dict) -> Optional[str]:
    """
    Standard spawn_bloom function that tick engine expects
    Wraps the enhanced_spawn_bloom with default pulse state
    ...
    try:
        ...
        if pulse:
            ...
            pulse_dict = {"heat": 0.0, "mood_pressure": {}}
    except:
        pulse_dict = {"heat": 0.0, "mood_pressure": {}}
    
    # Use enhanced spawn bloom with standard parameters
    return enhanced_spawn_bloom(bloom_data, pulse_dict, bypass_scup=False)

# Alias functions that other parts of the system might expect
def force_spawn_bloom(seed_id: str = "forced_bloom", **kwargs) -> str:
    """Force spawn a bloom for testing"""
    bloom_data = {
        "seed_id": seed_id,
        "lineage_depth": 1,
        "bloom_factor": 1.0,
    ...
    return result if result else f"[FAILED] {seed_id}"

def check_bloom_conditions(*args, **kwargs):
    """Placeholder for bloom condition checking"""
    # This is called by some systems but not implemented here
    return None

def get_bloom_stats():
    """Alias for get_bloom_statistics"""
    return get_bloom_statistics()

def print_bloom_statistics():
    """Print current bloom statistics"""
    stats = get_bloom_statistics()
    print("\n🌸 BLOOM STATISTICS:")
    for key, value in stats.items():
        print(f"  {key}: {value}")

def print_bloom_status(heat, scup, entropy, mood):
    ...
    print(f"  Mood: {mood}")
    print(f"  Statistics: {get_bloom_statistics()}")

def enable_bloom_debug(enabled: bool = True):
    """Enable/disable bloom debug mode"""
    global BLOOM_DEBUG
    BLOOM_DEBUG = enabled
    print(f"[SpawnBloom] Debug mode: {'enabled' if enabled else 'disabled'}")

# Module initialization message
print("[SpawnBloom] Enhanced spawn_bloom module loaded with tick engine compatibility")
