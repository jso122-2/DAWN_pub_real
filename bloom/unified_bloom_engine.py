...
class BloomEngine:
    ...
    def get_rebloom_queue_size(self) -> int:
        """Get current rebloom queue size for memory pressure calculation"""
        rebloom_count = 0
        
        # Count dormant blooms (potential reblooms)
        rebloom_count += len(self.dormant_blooms)
        
        # Count blooms awaiting reactivation
        if hasattr(self, 'activation_queue'):
            rebloom_count += len(getattr(self, 'activation_queue', []))
        
        # Count any failed blooms that might rebloom
        if hasattr(self, 'failed_blooms'):
            rebloom_count += min(self.failed_blooms, 5)  # Cap failed bloom contribution
        
        return rebloom_count
...
# Global helper function for memory manager
def get_rebloom_queue_size() -> int:
    """Global helper to get rebloom queue size"""
    # Try to get from global bloom engine instance
    try:
        from bloom.unified_bloom_engine import bloom_engine
        if hasattr(bloom_engine, 'get_rebloom_queue_size'):
            return bloom_engine.get_rebloom_queue_size()
    except:
        pass
    
    # Fallback: estimate from various bloom sources
    total_reblooms = 0
    try:
        from bloom.bloom_core.rebloom_router import REFRACTOR_QUEUE
        total_reblooms += len(REFRACTOR_QUEUE) if REFRACTOR_QUEUE else 0
    except:
        pass
    
    return total_reblooms