...
Usage:
    python dawn_integration.py [--config config.json] [--debug]
...
try:
    ...
    logging.warning("WebSocket dependencies not available. Install with: pip install websocket-client requests")
...
class MetricsStreamClient:
    ...
    def connect(self):
        ...
        if not WEBSOCKETS_AVAILABLE:
            logging.warning("WebSocket not available - metrics stream disabled")
            return
...
class DAWNTalkIntegration:
    ...
    def start(self):
        ...
        try:
            ...
            logging.info("📊 Metrics stream connected" if self.metrics_client and self.metrics_client.connected else "📊 Metrics stream offline")
